...
class Recorder:
    """A stand-in for ``transport.make_request`` that records every call and
...
def api(monkeypatch):
    rec = Recorder()
    monkeypatch.setattr(transport, "make_request", rec)
    return rec
...
def test_show_tool_table_queries_by_machine_id(api, capsys):
    cli.show_tool_table("machineid1")
...
def test_resolve_confirm_posts_to_instance_inbox(api, capsys):
    cli.resolve_pending("proposal", "confirm")
    post = api.last("POST")
...
def test_resolve_reject_posts_to_instance_inbox(api):
    cli.resolve_pending("proposal", "reject")
...
def test_delete_machine_is_path_based_no_body(api):
    cli.delete_machine("machineid1", assume_yes=True)
    d = api.last("DELETE")
...
def test_delete_tool_is_path_based_no_body(api):
    cli.delete_tool("instanceid1", assume_yes=True)
    d = api.last("DELETE")
...
def test_delete_entry_resolves_slot_then_deletes_by_record_id(api):
    cli.delete_entry("machineid1", 3, assume_yes=True)
    d = api.last("DELETE")
...
def test_bind_targets_slot_record_with_instance_id(api):
    cli.bind_entry("machineid1", 3, "instanceid1")
    post = api.last("POST")
...
def test_create_record_without_name_sends_empty_body(api):
    ...
    cli.create_record_from_entry("machineid1", 3)
...
def test_create_record_catalog_qa_passes_qa_payload_and_cert(api, tmp_path):
    ...
    cli.create_record_from_catalog("catalogid1", qa_path=str(qa),
...
def test_create_record_qa_without_cert_is_rejected(api, capsys, tmp_path):
    ...
    with pytest.raises(SystemExit):
        cli.create_record_from_catalog("catalogid1", qa_path=str(qa), cert=None)
...
def test_create_record_cert_without_qa_is_rejected(api, capsys):
    ...
    with pytest.raises(SystemExit):
        cli.create_record_from_catalog("catalogid1", qa_path=None,
                                           cert="kennametal@SN12345")
...
def test_resolve_slot_errors_when_tool_number_absent(api, capsys):
    with pytest.raises(SystemExit):
        cli.bind_entry("machineid1", 99, "instanceid1")
...
def test_create_machine_creates_then_asserts_name_and_controller(api, capsys):
    cli.create_machine("millstone", controller="linuxcnc")
    posts = api.of("POST")
...
def test_push_syncs_slots_with_parsed_fields(api):
    cli.push_table("machineid1", ["3:1/4 downcut:6.35", "7:chamfer"])
    sync = api.last("POST")
...
def test_push_snapshot_mode_is_explicit(api):
    cli.push_table("machineid1", ["3"], snapshot=True)
...
def test_client_add_to_set_appends_and_dedupes(api):
    cli.Client().add_to_set("setid1", ["instanceid1", "newtool99"])
    post = api.last("POST")
...
def test_client_add_to_set_preserves_existing_member_numbers(api):
    cli.Client().add_to_set("setid1", ["newtool99"])
...
def test_client_remove_from_set_drops_only_the_named_tool(api):
    cli.Client().remove_from_set("setid1", ["instanceid1"])
    post = api.last("POST")
    assert post["endpoint"] == "/tool-set-records/setid1/members"
    ids = [m["tool_record_id"] for m in post["body"]["members"]]
...
def test_add_to_set_cli_resolves_set_and_tools(api, capsys):
    cli.add_to_set("setid1", ["instanceid1"])
    post = api.last("POST")
    assert post["endpoint"] == "/tool-set-records/setid1/members"
...
def test_remove_from_set_cli_resolves_set_and_tools(api, capsys):
    cli.remove_from_set("setid1", ["instanceid1"])
    post = api.last("POST")
    assert post["endpoint"] == "/tool-set-records/setid1/members"
...
def test_create_catalog_record_posts_actor_and_fields(api, capsys, monkeypatch):
    ...
    cli.create_catalog_record(source="manufacturer:kennametal",
...
def test_create_catalog_record_surfaces_the_409_reuse_funnel(capsys, monkeypatch):
    ...
    def boom(*a, **k):
        raise cli.HTTPError(409, funnel)

    monkeypatch.setattr(transport, "make_request", boom)
    monkeypatch.setattr(cli.sys, "stdin", io.StringIO(""))
    with pytest.raises(SystemExit):
        cli._run(cli.create_catalog_record,
...
def test_show_catalog_record_resolves_and_shows_provenance(api, capsys):
    cli.show_catalog_record("catalogid1")
    out = capsys.readouterr().out
...
@pytest.mark.parametrize("invoke", [
    lambda: cli.list_machines(),
    lambda: cli.list_tools(),
    lambda: cli.list_tool_sets(),
    lambda: cli.list_pending(),
    lambda: cli.show_tool_table("machineid1"),
    lambda: cli.resolve_pending("proposal", "confirm"),
    lambda: cli.delete_machine("machineid1", assume_yes=True),
    lambda: cli.delete_tool("instanceid1", assume_yes=True),
    lambda: cli.delete_entry("machineid1", 3, assume_yes=True),
    lambda: cli.bind_entry("machineid1", 3, "instanceid1"),
    ...
    lambda: cli.remove_from_set("setid1", ["instanceid1"]),
    lambda: cli.show_tool_set("setid1"),
    lambda: cli.push_table("machineid1", ["3:1/4 downcut:6.35"]),
    lambda: cli.list_catalog_records(),
    lambda: cli.show_catalog_record("catalogid1"),
])
